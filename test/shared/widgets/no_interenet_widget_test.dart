...
class TestInternetStatusNotifier extends AutoDisposeStreamNotifier<InternetStatus>
...
void main() {
  ...
  group('No Internet Widget Test', () {
    ...
    testWidgets('renders the no internet widget child when internet is not available ',
        (tester) async {
      await tester.pumpApp(
        ProviderScope(
          overrides: [
          ...
          child: Scaffold(
            body: const Text('I am the child')
                .monitorConnection(noInternetWidget: const Text('I am no internet child')),
          ),
        ),
      );
      await tester.pumpAndSettle();
    ...
    testWidgets('renders the child when internet available without maintaining state ',
        (tester) async {
      await tester.pumpApp(
        ProviderScope(
          overrides: [
    ...
    testWidgets('renders error message  when any exception happened ', (tester) async {
      await tester.pumpApp(
        ProviderScope(
          overrides: [
            enableInternetCheckerPod.overrideWith(
      ...
      await tester.pumpAndSettle();
      expect(find.text("Unable to check internet due to Error in connection"), findsOneWidget);
    });
    ...
    testWidgets('renders child only on internet reconnected', (tester) async {
      ...
      container.read(internetCheckerNotifierPod.notifier).change(status: InternetStatus.connected);
    ...
    testWidgets('check no internet pod refreshed on ok clicked on snackbar', (tester) async {
      final ProviderContainer container = ProviderContainer(
        overrides: [
          enableInternetCheckerPod.overrideWith(
            (ref) => false,
      ...
      final okButton = find.byKey(const ValueKey('OK_BUTTON'), skipOffstage: false);
      await tester.ensureVisible(okButton);
      await tester.pumpAndSettle();

      await tester.tap(okButton);
      expect(container.read(internetCheckerNotifierPod).isRefreshing, isTrue);
      await tester.pumpAndSettle(const Duration(milliseconds: 500)); // Add a slight delay
    ...
    testWidgets('check no internet pod refreshed on ok clicked on snackbar when error occured',
        (tester) async {
      final ProviderContainer container = ProviderContainer(
        overrides: [
          enableInternetCheckerPod.overrideWith(
